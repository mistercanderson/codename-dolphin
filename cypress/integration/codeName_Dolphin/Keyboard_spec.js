describe('Keyboard', () => {
  ...
  it('Should respond to keyboard press', () => {
    ...
    cy.get('@color2')
    .then((color2) => {
      cy.get('@key').invoke('attr', 'style').should('not.eq', color2);
    });
  });

  it('Should respond to a click or touch', () => {
    cy.get('[note="D#4"]').should(
      'have.css',
      'background-color',
      'rgb(50, 58, 158)'
    );
